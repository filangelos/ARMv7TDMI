...
module MemoryInstructions =
    ...
    let storeInstructionW ((result: Input), (regN: AddressRegister), (state: MachineState)) =

        let resultVal = match result with
                       | ID R -> (^.) R state
                       | Literal x -> x
    ...
    let storeInstructionB ((result: Input), (regN: AddressRegister), (state: MachineState)) =
